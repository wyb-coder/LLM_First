...
openai.api_key = 'sk-C7gxtTx75u0V6kFJ4KflXYTVxtcNvuFyOMURIH0H9dG89XIU'
...
""".format(essay_prompt, essay)
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {"role": "system",
             "content": "As a virtual evaluator with expertise in English composition, your role is to critically analyze and grade student essays."},
...
""".format(rubrics, essay_prompt, examples, essay)
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {"role": "system",
             "content": "As a virtual evaluator with expertise in English composition, your role is to critically analyze and grade student essays according to a predetermined set of rubrics and graded examples."},
            {"role": "user", "content": prompt}
        ]
    )
...
def get_gpt_score(essay_set=1, prompt_func=zeroshot_worubrics_prompt):
    data = pd.read_excel("./data/training_set_rel33.xlsx")
