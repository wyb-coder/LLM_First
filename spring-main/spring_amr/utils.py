...
def instantiate_model_and_tokenizer(
    ...
    if from_pretrained:
        model = AMRBartForConditionalGeneration.from_pretrained(name, config=config) # name = bart-large
    else:
    ...
    if checkpoint is not None:
        model.load_state_dict(torch.load(checkpoint, map_location='cpu')['facebook'])
